	...
	class Shoestrap_Blog {
		...
		function archives_full_footer() { ?>
			<footer style="margin-top: 2em;">
				<i class="kp-folder-open3"></i> <?php _e( 'Categories: ', 'knowledgepress' ); ?>
				<span class="label label-tag">
					<?php echo get_the_category_list( '</span> ' . '<span class="label label-tag">' ); ?>
				</span>

				<?php echo get_the_tag_list( '<i class="kp-price-tags2"></i> ' . __( 'Tags: ', 'knowledgepress' ) . '<span class="label label-tag">', '</span> ' . '<span class="label label-tag">', '</span>' ); ?>
		...
		function meta_custom_render() {
			...
			if ( is_archive() && $ss_settings['archive_meta'] == 0 ) {
				return;
			}
			...
			if ( is_array( $metas ) ) {
				foreach ( $metas as $meta => $value ) {
					...
					if ( $meta == 'sticky' && ! empty( $value ) && is_sticky() ) {
						$content .= $ss_framework->open_col( 'span', array( 'medium' => $col ), null, 'featured-post' ) . '<i class="kp-flag8"></i> ' . __( 'Sticky', 'knowledgepress' ) . $ss_framework->close_col( 'span' );
					}

					// output post format element
					if ( $meta == 'post-format' && ! empty( $value ) ) {
						if ( get_post_format( $post->ID ) === 'gallery' ) {
						  $content .= $ss_framework->open_col( 'span', array( 'medium' => $col ), null, 'post-format' ) . '<i class="kp-file-text2"></i> <a href="' . esc_url( get_post_format_link( 'gallery' ) ) . '">' . __('Gallery','knowledgepress') . '</a>' . $ss_framework->close_col( 'span' );
						} elseif ( get_post_format( $post->ID ) === 'aside' ) {
						  $content .= $ss_framework->open_col( 'span', array( 'medium' => $col ), null, 'post-format' ) . '<i class="kp-file-text2"></i> <a href="' . esc_url( get_post_format_link( 'aside' ) ) . '">' . __('Aside','knowledgepress') . '</a>' . $ss_framework->close_col( 'span' );
						} elseif ( get_post_format( $post->ID ) === 'link' ) {
						  $content .= $ss_framework->open_col( 'span', array( 'medium' => $col ), null, 'post-format' ) . '<i class="kp-link2"></i> <a href="' . esc_url( get_post_format_link( 'link' ) ) . '">' . __('Link','knowledgepress') . '</a>' . $ss_framework->close_col( 'span' );
						} elseif ( get_post_format( $post->ID ) === 'image' ) {
						  $content .= $ss_framework->open_col( 'span', array( 'medium' => $col ), null, 'post-format' ) . '<i class="kp-image4"></i> <a href="' . esc_url( get_post_format_link( 'image' ) ) . '">' . __('Image','knowledgepress') . '</a>' . $ss_framework->close_col( 'span' );
						} elseif ( get_post_format( $post->ID ) === 'quote' ) {
						  $content .= $ss_framework->open_col( 'span', array( 'medium' => $col ), null, 'post-format' ) . '<i class="kp-file-text2"></i> <a href="' . esc_url( get_post_format_link( 'quote' ) ) . '">' . __('Quote','knowledgepress') . '</a>' . $ss_framework->close_col( 'span' );
						} elseif ( get_post_format( $post->ID ) === 'status' ) {
						  $content .= $ss_framework->open_col( 'span', array( 'medium' => $col ), null, 'post-format' ) . '<i class="kp-file-text2"></i> <a href="' . esc_url( get_post_format_link( 'status' ) ) . '">' . __('Status','knowledgepress') . '</a>' . $ss_framework->close_col( 'span' );
						} elseif ( get_post_format( $post->ID ) === 'video' ) {
						  $content .= $ss_framework->open_col( 'span', array( 'medium' => $col ), null, 'post-format' ) . '<i class="kp-film3"></i> <a href="' . esc_url( get_post_format_link( 'video' ) ) . '">' . __('Video','knowledgepress') . '</a>' . $ss_framework->close_col( 'span' );
						} elseif ( get_post_format( $post->ID ) === 'audio' ) {
						  $content .= $ss_framework->open_col( 'span', array( 'medium' => $col ), null, 'post-format' ) . '<i class="kp-volume-medium3"></i> <a href="' . esc_url( get_post_format_link( 'audio' ) ) . '">' . __('Audio','knowledgepress') . '</a>' . $ss_framework->close_col( 'span' );
						} elseif ( get_post_format( $post->ID ) === 'chat' ) {
						  $content .= $ss_framework->open_col( 'span', array( 'medium' => $col ), null, 'post-format' ) . '<i class="kp-file-text2"></i> <a href="' . esc_url( get_post_format_link( 'chat' ) ) . '">' . __('Chat','knowledgepress') . '</a>' . $ss_framework->close_col( 'span' );
						}
					}
					...
					if ( $meta == 'category' && ! empty( $value ) ) {
						if ( $categories_list ) {
							$content .= $ss_framework->open_col( 'span', array( 'medium' => $col ), null, 'categories-links' ) . '<i class="kp-folder-open3"></i> ' . $categories_list . $ss_framework->close_col( 'span' );
						}
					}
					...
					if ( $meta == 'tags' && ! empty( $value ) ) {
						if ( $tag_list ) {
							$content .= $ss_framework->open_col( 'span', array( 'medium' => $col ), null, 'tags-links' ) . '<i class="kp-price-tags2"></i> ' . $tag_list . $ss_framework->close_col( 'span' );
						}
					}

					// output author element
					if ( $meta == 'author' && ! empty( $value ) ) {
						$content .= sprintf( $ss_framework->open_col( 'span', array( 'medium' => $col ), null, 'author vcard' ) . '<i class="kp-user2"></i> <a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a>' . $ss_framework->close_col( 'span' ),
					...
					if ( $meta == 'comment-count' && ! empty( $value ) ) {
						if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
							$content .= $ss_framework->open_col( 'span', array( 'medium' => $col ), null, 'comments-link' ) . '<i class="kp-bubble2"></i> <a href="' . get_comments_link( $post->ID ) . '">' . get_comments_number( $post->ID ) . ' ' . __( 'Comments', 'knowledgepress' ) . '</a>' . $ss_framework->close_col( 'span' );
						}
					}
