...
function shoestrap_bbp_user_subscribe_link( $args = '', $user_id = 0, $wrap = true ) {
	...
	function shoestrap_bbp_get_user_subscribe_link( $args = '', $user_id = 0, $wrap = true ) {
		...
		// Parse arguments against default values
		$r = bbp_parse_args( $args, array(
			'subscribe'   => __( 'Subscribe',   'knowledgepress' ),
			'unsubscribe' => __( 'Unsubscribe', 'knowledgepress' ),
			'user_id'     => 0,
			'topic_id'    => 0,
			'before'      => '&nbsp;|&nbsp;',
			'after'       => ''
...
function shoestrap_bbp_user_favorites_link( $args = array(), $user_id = 0, $wrap = true ) {
	...
	function shoestrap_bbp_get_user_favorites_link( $args = '', $user_id = 0, $wrap = true ) {
		...
		// Parse arguments against default values
		$r = bbp_parse_args( $args, array(
			'favorite'  => __( 'Favorite',  'knowledgepress' ),
			'favorited' => __( 'Favorited', 'knowledgepress' ),
			'user_id'   => 0,
			'topic_id'  => 0,
			'before'    => '',
			'after'     => ''
...
function shoestrap_bbp_dropdown( $args = '' ) {
	 ...
	 *  - show_none: False or something like __( '(No Forum)', 'knowledgepress' ),
	 *                will have value=""
	 *  - none_found: False or something like
	 *                 __( 'No forums to post to!', 'knowledgepress' )
		...
		// No items found - Display feedback if no custom message was passed
			...
			switch ( $r['post_type'] ) {
				...
				case bbp_get_topic_post_type() :
					$retval = __( 'No topics available', 'knowledgepress' );
					break;

				// Forums
				case bbp_get_forum_post_type() :
					$retval = __( 'No forums available', 'knowledgepress' );
					break;

				// Any other
				default :
					$retval = __( 'None available', 'knowledgepress' );
					break;
			}
		}
