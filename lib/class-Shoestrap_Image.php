	...
	class Shoestrap_Image {
		...
		public static function image_resize( $data ) {
			...
			if ( current_theme_supports( 'retina' ) && empty( $settings['retina'] ) ) {
				$results['retina'] = self::_resize( $settings['abs_url'], $settings['url'], $settings['width'], $settings['height'], $settings['crop'], true );
			}

			return self::_resize( $settings['abs_url'], $settings['url'], $settings['width'], $settings['height'], $settings['crop'], false );    
		}
		 ...
		 */
		public static function _resize( $abs_url, $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
			global $wpdb;

			if ( empty( $url ) ) {
				return new WP_Error( 'no_image_url', __( 'No image URL has been entered.', 'knowledgepress' ), $url );
			}
